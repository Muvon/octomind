...
fn parse_rule_line(line: &str) -> Vec<ActivateCheck> {
	...
	while !rest.is_empty() {
		...
		// Scan for the paren that CLOSES this check, not the first `)` in the
		// line: check args legitimately nest parens (`match(\b(deploy|ship)\b)`),
		// and cutting at the first one hands ActivateCheck a truncated regex
		// that compiles to nothing and never fires.
		...
		if let Some(check) = ActivateCheck::parse(&rest[..=close]) {
			checks.push(check);
		}
