...
fn test_max_lines_one_keeps_only_marker() {
	...
	assert_eq!(
		result,
		vec!["[4 lines truncated - use more specific patterns or increase max_lines]".to_string()]
	);
	assert_eq!(
		info.as_deref(),
...
fn test_two_lines_with_max_one_reports_both_truncated() {
	...
	assert_eq!(
		result,
		vec!["[2 lines truncated - use more specific patterns or increase max_lines]".to_string()]
	);
	assert_eq!(
		info.as_deref(),
