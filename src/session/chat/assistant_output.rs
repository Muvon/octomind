...
// Match <system>...</system> (multiline, lazy) including surrounding blank lines.
static SYSTEM_TAG_REGEX: LazyLock<Regex> = LazyLock::new(|| {
	Regex::new(r"(?is)\s*<system>.*?</system>\s*").expect("valid system-tag regex")
});
