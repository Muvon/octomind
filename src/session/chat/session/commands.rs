	...
	pub async fn process_command(
		...
		match command {
			...
			REDUCE_COMMAND => {
				...
				match self.handle_reduce_command(config).await {
					Ok(()) => {
						...
						println!(
							...
							format!(
								...
								self.session.messages.len() - 1, // -1 for system message
								format_number(tokens_saved as u64),
								format_number(new_tokens as u64)
							)
							.bright_green()
...
allowed_tools = []"#
	...
	// Reduction handler method
	async fn handle_reduce_command(&mut self, config: &Config) -> Result<()> {
		...
		for (i, message) in self.session.messages.iter().enumerate() {
		...
		self.session.messages.retain(|msg| msg.role == "system");

		// Add the reduced content as a single user message
		self.add_user_message(&reduced_content)?;
