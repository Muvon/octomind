...
/// requests, so it occupies context the model actually reads. Answered by the
/// provider itself (`AiProvider::replays_thinking`), which owns the request
/// builder that does or does not replay it. Counting dropped thinking towards
/// the context estimate made a 60k-token prompt read as 100k (measured on
/// deepseek-v4-flash via Alibaba), so folds fired at half the fire line and
/// the "tokens saved" they reported were mostly text the provider never saw.
/// An unresolvable model (unknown provider, unsupported name) counts nothing.
pub fn model_replays_thinking(model: &str) -> bool {
	ProviderFactory::get_provider_for_model(model)
		.map(|(provider, actual_model)| provider.replays_thinking(&actual_model))
		.unwrap_or(false)
}
